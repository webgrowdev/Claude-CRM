...
// Column definitions with ALL FunnelStatus values
const columns: { id: FunnelStatus; title: string; color: string; borderColor: string; bgColor: string }[] = [
  ...
  { id: 'appointment', title: 'Turno Agendado', color: '#8B5CF6', borderColor: 'border-t-purple-500', bgColor: 'bg-purple-50/50' },
  { id: 'attended', title: 'Asistió', color: '#10B981', borderColor: 'border-t-emerald-500', bgColor: 'bg-emerald-50/50' },
  { id: 'closed', title: 'Cerrado', color: '#22C55E', borderColor: 'border-t-success-500', bgColor: 'bg-success-50/50' },
  { id: 'followup', title: 'Seguimiento', color: '#06B6D4', borderColor: 'border-t-cyan-500', bgColor: 'bg-cyan-50/50' },
  { id: 'noshow', title: 'No Asistió', color: '#EF4444', borderColor: 'border-t-red-500', bgColor: 'bg-red-50/50' },
...
export function KanbanView({ patients, onPatientClick, className }: KanbanViewProps) {
  const { updatePatientStatus } = useApp()
  ...
  const patientsByStatus = useMemo(() => {
    ...
    patients.forEach((patient) => {
      // Map legacy 'scheduled' status to 'appointment'
      const status = patient.status === 'scheduled' ? 'appointment' : patient.status
      if (grouped[status as FunnelStatus]) {
        grouped[status as FunnelStatus].push(patient)
      }
    })
