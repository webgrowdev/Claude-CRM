...
import { formatTimeAgo, formatRelativeDate, getStatusLabel, formatCurrency, getWhatsAppUrl, getPhoneUrl, cn } from '@/lib/utils'
...
export default function DashboardPage() {
  ...
  // Smart insights - actionable items
  const insights = useMemo(() => {
    ...
    const urgentLeads = state.leads.filter(l =>
      l.status === 'new' &&
      new Date(l.createdAt) < fortyEightHoursAgo
    ).slice(0, 3)

    // Appointments needing confirmation (scheduled but not confirmed)
    const unconfirmedAppointments = state.leads.reduce((acc, lead) => {
      const unconfirmed = lead.followUps.filter(fu =>
        (fu.type === 'meeting' || fu.type === 'appointment') &&
        !fu.completed &&
        !fu.confirmedByPatient &&
        new Date(fu.scheduledAt) > now
      )
      return [...acc, ...unconfirmed.map(fu => ({ lead, followUp: fu }))]
    }, [] as { lead: Lead; followUp: any }[]).slice(0, 3)

    // Overdue follow-ups
    const overdueFollowUps = state.leads.reduce((acc, lead) => {
      const overdue = lead.followUps.filter(fu =>
        !fu.completed &&
        new Date(fu.scheduledAt) < now
      )
      return [...acc, ...overdue.map(fu => ({ lead, followUp: fu }))]
    }, [] as { lead: Lead; followUp: any }[]).slice(0, 3)

    return { hotLeads, urgentLeads, unconfirmedAppointments, overdueFollowUps }
  }, [state.leads, state.treatments])

  // Color class mapping for Tailwind JIT
  const colorClasses = {
  ...
  const statsCards = [
    {
      label: language === 'es' ? 'Pacientes Nuevos' : 'New Patients',
      value: stats.newLeads,
      change: language === 'es' ? 'Sin contactar' : 'Not contacted',
      colorKey: 'primary' as const,
      icon: UserRound,
      href: '/pacientes?status=new',
    },
    {
      label: language === 'es' ? 'Citas Hoy' : 'Today\'s Appointments',
      value: stats.todayAppointments,
      change: language === 'es' ? 'Programadas' : 'Scheduled',
    ...
    },
    {
      ...
      change: language === 'es' ? 'Tasa actual' : 'Current rate',
  ...
  const getFollowUpIcon = (type: string) => {
    switch (type) {
      case 'call': return <Phone className="w-4 h-4 text-primary-500" />
      case 'message': return <MessageCircle className="w-4 h-4 text-success-500" />
  ...
  const hasInsights = insights.hotLeads.length > 0 ||
  ...
  return (
    <AppShell>
      ...
      <PageContainer>
        {/* Smart Action Cards - Priority Insights */}
        {hasInsights && (
          <div className="mb-6">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-orange-800">
                        {insights.hotLeads.length} {language === 'es' ? 'Leads Calientes' : 'Hot Leads'}
                      </h3>
                      <p className="text-xs text-orange-600">
                        {language === 'es' ? 'Alta probabilidad de conversión' : 'High conversion probability'}
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {insights.hotLeads.map((lead) => (
                      <div key={lead.id} className="flex items-center justify-between bg-white/60 rounded-xl p-3">
                        <div className="flex items-center gap-3">
                          <Avatar name={lead.name} size="sm" />
                          <div>
                            <p className="font-medium text-slate-800 text-sm">{lead.name}</p>
                            <div className="flex items-center gap-2">
                              <LeadScoreBadge score={lead.score.total} size="sm" showLabel={false} />
                              <span className="text-xs text-slate-500">
                                {lead.treatments[0] || (language === 'es' ? 'Sin tratamiento' : 'No treatment')}
                              </span>
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-red-800">
                        {insights.urgentLeads.length} {language === 'es' ? 'Sin Contactar' : 'Not Contacted'}
                      </h3>
                      <p className="text-xs text-red-600">
                        {language === 'es' ? 'Más de 48 horas esperando' : 'Waiting for 48+ hours'}
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-amber-800">
                        {insights.overdueFollowUps.length} {language === 'es' ? 'Seguimientos Vencidos' : 'Overdue Follow-ups'}
                      </h3>
                      <p className="text-xs text-amber-600">
                        {language === 'es' ? 'Requieren atención inmediata' : 'Need immediate attention'}
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-slate-800 text-sm">{lead.name}</p>
                            <p className="text-xs text-amber-600">
                              {language === 'es' ? 'Programado para' : 'Scheduled for'} {formatRelativeDate(new Date(followUp.scheduledAt))}
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      <h3 className="font-semibold text-purple-800">
                        {insights.unconfirmedAppointments.length} {language === 'es' ? 'Por Confirmar' : 'To Confirm'}
                      </h3>
                      <p className="text-xs text-purple-600">
                        {language === 'es' ? 'Citas sin confirmación del paciente' : 'Unconfirmed appointments'}
                      </p>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {insights.unconfirmedAppointments.map(({ lead, followUp }) => (
                      <div key={followUp.id} className="flex items-center justify-between bg-white/60 rounded-xl p-3">
                        <div className="flex items-center gap-3">
                          <Avatar name={lead.name} size="sm" />
                          <div>
                            <p className="font-medium text-slate-800 text-sm">{lead.name}</p>
                        ...
                        <a
                          href={getWhatsAppUrl(lead.phone, language === 'es'
        ...
        <div className="lg:grid lg:grid-cols-2 lg:gap-6">
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base lg:text-lg font-semibold text-slate-800">
                {language === 'es' ? 'Pacientes Recientes' : 'Recent Patients'}
              </h2>
              ...
              {recentLeads.length === 0 ? (
                <div className="py-8 text-center">
                  ...
                  <p className="text-slate-500">
                    {language === 'es' ? 'No hay pacientes todavía' : 'No patients yet'}
                  </p>
                  <Link
                    href="/pacientes?action=new"
                    className="inline-block mt-3 text-sm text-primary-600 font-medium hover:underline"
                  >
                    {language === 'es' ? 'Agregar primer paciente' : 'Add first patient'}
                  </Link>
                </div>
              ) : (
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-base lg:text-lg font-semibold text-slate-800">
                {language === 'es' ? 'Próximos Seguimientos' : 'Upcoming Follow-ups'}
              </h2>
            ...
            <Card padding="none">
              {upcomingFollowUps.length === 0 ? (
                <div className="py-8 text-center">
                  ...
                  <p className="text-slate-500">
                    {language === 'es' ? 'No hay seguimientos programados' : 'No scheduled follow-ups'}
                  </p>
                  <Link
                    href="/pacientes"
                    className="inline-block mt-3 text-sm text-primary-600 font-medium hover:underline"
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-slate-500">
                          {t.followUp[`type${followUp.type.charAt(0).toUpperCase() + followUp.type.slice(1)}`] || followUp.type}
            ...
            <div className="mt-4 pt-4 border-t border-white/20">
              <div className="flex justify-between text-sm lg:text-base">
                <span className="text-primary-100">
                  {state.leads.filter(l => l.status === 'closed').length} {language === 'es' ? 'ventas cerradas' : 'closed sales'}
                </span>
